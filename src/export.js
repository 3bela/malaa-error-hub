 ...
 * 1. Error Code
 * 2. Service
 * 3. Original AR Message
 * 4. Original EN Message
 * 5. Corrected AR Message
 * 6. Corrected EN Message
 */
...
function generateMarkdown(records) {
  ...
  md.push("");
  md.push("| Error Code | Service | Original AR Message | Original EN Message | Corrected AR Message | Corrected EN Message |");
  md.push("| :--- | :--- | :--- | :--- | :--- | :--- |");
  ...
  records.forEach(function(r) {
    ...
    md.push("| `" + esc(r.errorCode) + "` | " + esc(r.service) + " | " + esc(r.originalArMessage) + " | " + esc(r.originalEnMessage) + " | **" + esc(r.correctedArMessage || "—") + "** | **" + esc(r.correctedEnMessage || "—") + "** |");
  });
...
function generateHtml(records) {
  ...
  html.push("    <div class=\"meta\">Exported Records: " + records.length + " | 6 Core Columns (Error Code, Service, Original AR, Original EN, Corrected AR, Corrected EN)</div>");
  ...
  html.push("            <th class=\"arabic\">Original AR Message</th>");
  html.push("            <th>Original EN Message</th>");
  html.push("            <th class=\"arabic\">Corrected AR Message</th>");
