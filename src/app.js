...
var AppState = {
  ...
  engModalErrorId: null,  // For mark single as implemented confirmation modal
...
function showToast(message, type) {
  ...
  toastEl.innerHTML = `
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ${type === "success" ? '<polyline points="20 6 9 17 4 12"></polyline>' : (type === "warning" ? '<path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line>')}
    </svg>
...
function renderStatusLegend() {
  return `
    <div class="status-legend-bar">
      <span class="status-legend-title">Status Workflow:</span>
      <div class="status-legend-item" title="Customer Support submitted a new request related to an error.">
        ...
        <span><strong>Change Request CS</strong>: CS submitted request</span>
      </div>
      <div class="status-legend-item" title="An error was extracted from the code but Product has not reviewed it.">
        ...
        <span><strong>Not Reviewed</strong>: Extracted; unreviewed</span>
      </div>
      <div class="status-legend-item" title="Product is currently reviewing or correcting the error.">
        ...
        <span><strong>In Review</strong>: Product refining copy</span>
      </div>
      <div class="status-legend-item" title="Product approved the corrections and the error is ready for Engineering.">
        ...
        <span><strong>Ready for Engineering</strong>: Approved for code</span>
      </div>
      <div class="status-legend-item" title="Engineering confirmed that the correction was applied in the code.">
        ...
        <span><strong>Implemented</strong>: Live in code</span>
      </div>
    </div>
  `;
}
...
function renderApp() {
  ...
  // 1. Header with 3 Perspectives (Product, Customer Care, Engineer) & Tools
  html += `
    ...
    if (pov === "product") {
      ...
      html += `
        ...
        <li><button type="button" class="nav-link-btn product ${AppState.productTab === 'cs-requests' ? 'active product' : ''}" onclick="setProductTab('cs-requests')">
          ...
          Customer Care Requests ${kpis.changeRequestCS > 0 ? `<span style="background:#d97706;color:#ffffff;font-size:0.68rem;padding:0.1rem 0.4rem;border-radius:10px;font-weight:700;margin-left:0.25rem;">${kpis.changeRequestCS}</span>` : ''}
        </button></li>
  ...
  html += `
    <footer class="app-footer">
      <p>Malaa Error Hub &bull; 3 Perspectives (Product, Customer Care, Engineer) &bull; 5 Final Statuses &bull; Local Storage</p>
    </footer>
  `;
...
// ==========================================================================
// 1. Role Selection Landing Page (3 Perspectives: Product, Customer Care, Engineer)
// ==========================================================================
      ...
      <div class="role-cards-grid-3">
        ...
        <div class="role-card engineering-card" onclick="setPOV('engineering')">
          ...
          <div class="role-card-content">
            ...
            <p class="role-card-desc">
              Receive Product-approved corrections, export 6-column files (CSV, MD, HTML), and confirm code fixes.
            </p>
            <div class="role-card-features">
              <div class="role-feature-item">&bull; Dedicated Ready for Engineering Queue</div>
              <div class="role-feature-item">&bull; Implement Selected errors in codebase</div>
              <div class="role-feature-item">&bull; Return Implemented errors to Ready for Engineering</div>
              <div class="role-feature-item">&bull; Export 6 Core Columns (CSV, MD, HTML)</div>
            </div>
          </div>
          <div class="role-card-action">
      ...
      <div class="kpi-grid">
        ...
        <div class="kpi-card kpi-change_request_cs" onclick="handleKpiClick('change_request_cs')" title="View Customer Care requests">
          <div class="kpi-content">
            ...
            <span class="kpi-value" style="color:#b45309;">${kpis.changeRequestCS}</span>
          </div>
          <div class="kpi-icon">
        ...
        <div class="kpi-card kpi-implemented" onclick="handleKpiClick('implemented')" title="View errors confirmed implemented">
          <div class="kpi-content">
            <span class="kpi-label">Implemented</span>
            <span class="kpi-value" style="color:#047857;">${kpis.implemented}</span>
          </div>
          <div class="kpi-icon">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
          </div>
...
function renderProductAllErrorsPage() {
  ...
  var html = `
    <div style="display:flex;flex-direction:column;gap:1.15rem;">
      <div>
        <h2 style="font-size:1.35rem;font-weight:800;color:var(--navy-dark);">All Errors Master Catalog</h2>
        <p style="font-size:0.85rem;color:var(--text-secondary);">
          Browse the complete catalog of application error definitions, bilingual copy, and current review statuses.
        </p>
      </div>
...
// ==========================================================================
// 5. Product POV — Customer Care Requests (Active Inbox, Includes Product Response)
// ==========================================================================

function getFilteredCSRequests() {
  var records = errorStore.getAll();
  // Active CS Requests: show items with feedback
  var requests = records.filter(function(r) {
    return !!r.customerSupportComment && (r.status === "change_request_cs" || r.status === "in_review" || r.status === "not_reviewed");
  });
...
function renderCSRequestsTableRows(requests) {
  if (requests.length === 0) {
    return '<tr><td colspan="7" style="text-align:center;padding:2.5rem;color:var(--text-muted);">No active Customer Care requests pending review. All approved items are moved to Engineering queue.</td></tr>';
  }
...
function renderCSRequestsPage() {
  ...
  var html = `
    <div style="display:flex;flex-direction:column;gap:1.15rem;">
      <div>
        <h2 style="font-size:1.35rem;font-weight:800;color:var(--navy-dark);">Customer Care Requests</h2>
        <p style="font-size:0.85rem;color:var(--text-secondary);">
          Review feedback submitted by Customer Support, write Product responses, and prepare approved corrections for Engineering.
        </p>
      </div>
      ...
      <div class="filter-bar-card">
        ...
        <div style="font-size:0.8rem;color:var(--text-muted);">
          <span id="csRequestsCounter">Pending CS Feedback Items: <strong>${requests.length}</strong></span>
        </div>
      </div>
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th style="width:26%;">Support Comment</th>
                <th style="width:24%;">Product Response</th>
                <th style="width:140px;">Current Status</th>
                <th style="width:135px;text-align:right;">Action</th>
              </tr>
            </thead>
...
function handleCSRequestsSearchInput(val) {
  ...
  if (counter) counter.innerHTML = "Pending CS Feedback Items: <strong>" + requests.length + "</strong>";
}
...
function renderModals() {
          ...
          <div class="modal-dialog" onclick="event.stopPropagation()">
            ...
            <div class="modal-body">
              <p style="font-size:0.825rem;color:var(--text-secondary);">
                Submit feedback to Product to update error copy or operational details. This will set the error status to <strong>Change Request CS</strong>.
              </p>
  ...
  // 2. Engineering Confirm Implemented Modal (No Source Reference)
            ...
            <div class="modal-body">
              ...
              <div style="background:#ecfdf5;border:1px solid #a7f3d0;padding:0.75rem 1rem;border-radius:var(--radius-sm);display:flex;flex-direction:column;gap:0.4rem;">
                <div>
                  ...
                  <div class="arabic" style="font-weight:600;direction:rtl;text-align:right;font-size:0.95rem;">${escapeHtml(engRecord.correctedArMessage)}</div>
                </div>
                <div style="margin-top:0.35rem;">
                  <span style="font-size:0.7rem;font-weight:700;color:#047857;text-transform:uppercase;">Corrected English (Live):</span>
                  <div style="font-weight:600;font-size:0.85rem;">${escapeHtml(engRecord.correctedEnMessage)}</div>
                </div>
              </div>

              <p style="font-size:0.775rem;color:var(--text-muted);">
                This action will update the shared status to <strong>Implemented</strong> and immediately display the corrected copy in Customer Care.
              </p>
            </div>
            <div class="modal-footer">
